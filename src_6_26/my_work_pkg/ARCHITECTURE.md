...
/map ────────────┘         │
     ...
     tf: odom → map
              ...
              (位置 x,y 做 2D 刚体变换)
                    │
                    ├── source=snapped: 在新 /map 上小半径 re-snap
...
- `source=snapped`：对象已经和 OccupancyGrid 结构绑定，先做几何修正，再以修正后的位置为种子，在新 `/map` 上用小半径重新吸附，刷新对象中心与尺寸。
...
| `lc_resnap_radius_m` | 0.3 | snapped 对象 re-snap 搜索半径 (米) |
...
| `lc_resnap_radius_m` | 0.3 | 回环后 snapped 对象重吸附半径 | 太小可能找不到新地图占据格；太大可能吸到邻近墙体 |
