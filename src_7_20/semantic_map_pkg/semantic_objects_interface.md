...
| `raw_seed_visualizer_node` | 将吸附前的 `/semantic_seed` 发布为原始点 Marker，供前后位置对比 |
...
```json
{
  ...
  "pose": {
    ...
    "action_tags": ["waving"],
    "keypoints": [
      {"name": "nose", "x": 318.2, "y": 104.5, "confidence": 0.94}
    ]
  }
}
```
...
`pose` 由 `frontend_bridge_node` 将 person 检测框和 pose 检测框按 IoU
关联后写入。投影节点把它视为非几何语义：地图吸附只修改对象位置和尺寸，
不会丢弃或改写 posture、action tags 和 keypoints。
...
- 对象存在姿态时，文本 Marker 追加显示最新 `posture` 和 `action_tags`。
...
| `last_seen` | 最近一次观测时间 |
| `created_at` | 第一次创建时间 |
| `pose` | 可选，最近一次有效人体姿态摘要 |
| `pose_last_seen` | 可选，最近一次收到有效 pose 的时间，单位秒 |

姿态更新规则：

- 同一对象收到新的有效 `pose` 时，整体替换为最新姿态，不做 EMA。
- 某一帧 person 没有匹配到 pose 时，保留上一次有效姿态。
- `pose_last_seen` 不会在缺失 pose 的普通检测帧中刷新，上层可据此判断姿态是否陈旧。
- 回环修正只更新对象几何属性，不修改姿态属性。
...
| `lc_translation_threshold` | launch 默认 0.20 | 位移跳变阈值 |
| `lc_rotation_threshold` | launch 默认 0.08 | 旋转跳变阈值 |
| `lc_source_frame` | odom | 通过 `lookup_transform("map", lc_source_frame)` 监控 |
| `lc_resnap_enabled` | true | 回环后是否对 `snapped` 对象 re-snap |
| `lc_resnap_radius_m` | 0.3 | 第一次 re-snap 搜索半径 |
| `lc_resnap_max_attempts` | 4 | 失败对象最多等待多少个新地图帧重试 |
| `lc_resnap_radius_step_m` | 0.15 | 每轮失败后的搜索半径增量 |
| `lc_resnap_max_radius_m` | 0.6 | re-snap 搜索半径上限 |
| `lc_stable_checks` | 2 | 新地图到达后，re-snap 前要求 TF 连续稳定的检查次数 |
| `lc_stable_translation_threshold` | 0.02 | 单次稳定检查允许的最大平移变化，单位米 |
| `lc_stable_rotation_threshold` | 0.01 | 单次稳定检查允许的最大旋转变化，单位弧度 |
...
检测到 TF jump
  -> 所有对象立即做几何修正
  -> visual_only 处理完成
  -> snapped 对象进入 pending_resnap
  -> 等待回环检测之后的新一帧 /map
  -> 等待 map<-odom 连续稳定 lc_stable_checks 次
  -> 使用新地图进行小半径 re-snap
  -> 失败对象等待下一帧 /map，并逐步扩大半径重试
```

新地图和 TF 稳定条件可以按任意顺序到达，只有两者同时满足才会执行一次
re-snap。re-snap 失败时保留几何修正位置，不修改 `times_seen`、`confidence`、
`last_seen`、`state` 或姿态信息。

默认重试半径为 `0.30 -> 0.45 -> 0.60 -> 0.60 m`。同一地图 generation
最多尝试一次，成功对象立即移出 pending；达到最大次数后才放弃并打印对象 ID。
...
使用 `ros2 topic echo /semantic_seed` 检查投影坐标、标签和姿态字段。
...
- `pose.keypoints` 仍是输入图像像素坐标，不是 `map` 坐标；上层使用前应结合 `pose_last_seen` 判断时效性。
