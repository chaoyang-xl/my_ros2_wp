...
class FrontendBridgeNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("max_depth_time_diff_s", 0.15)
        self.declare_parameter("fallback_latest_depth_without_stamp", True)
        self.declare_parameter("use_image_stamp_for_tf", False)
        # Pose matching
        ...
        self._max_depth_time_diff_s = float(
            self.get_parameter("max_depth_time_diff_s").value
        )
        self._fallback_latest_depth_without_stamp = self._get_bool_parameter(
            "fallback_latest_depth_without_stamp"
        )
        self._use_image_stamp_for_tf = self._get_bool_parameter(
            "use_image_stamp_for_tf"
        )
        self._pose_iou_threshold = self.get_parameter("pose_iou_threshold").value
        self._enable_pose = self._get_bool_parameter("enable_pose")
        ...
        depth_buffer_size = int(self.get_parameter("depth_buffer_size").value)
        self._depth_buffer = deque(maxlen=max(1, depth_buffer_size))
        self._depth_buffer_initialized = False
    ...
    def _depth_cb(self, msg: Image) -> None:
        ...
        if not self._depth_buffer_initialized:
            self._depth_buffer_initialized = True
            self.get_logger().info(
                "Depth buffer initialized: "
                f"topic={self.get_parameter('depth_topic').value}, "
                f"frame={msg.header.frame_id}, size={msg.width}x{msg.height}, "
                f"encoding={msg.encoding}"
            )

    def _json_cb(self, json_msg: String) -> None:
        if not self._depth_buffer:
            self.get_logger().warn("Received JSON but no depth image yet")
            return
        ...
        json_stamp = self._extract_payload_stamp(payload)
        if json_stamp is None:
            if not self._fallback_latest_depth_without_stamp:
                self.get_logger().warn(
                    "JSON has no stamp_sec/stamp_nanosec and latest-depth fallback is disabled"
                )
                return
            self.get_logger().debug("JSON has no stamp; using latest depth frame")
            self._synced_cb(json_msg, self._depth_buffer[-1])
            return

        best_depth_msg = None
        best_dt_s = float("inf")
        for depth_msg in self._depth_buffer:
            dt_s = abs(self._stamp_to_seconds(depth_msg.header.stamp) - json_stamp)
            if dt_s < best_dt_s:
                best_dt_s = dt_s
                best_depth_msg = depth_msg

        if best_depth_msg is None:
            self.get_logger().warn("Received JSON but no depth image yet")
            return

        if best_dt_s > self._max_depth_time_diff_s:
            self.get_logger().warn(
                "No matching depth for JSON stamp; "
                f"best_dt={best_dt_s:.3f}s > {self._max_depth_time_diff_s:.3f}s"
            )
            return

        self.get_logger().debug(
            "Matched JSON to depth: "
            f"dt={best_dt_s:.3f}s, "
            f"depth_stamp={best_depth_msg.header.stamp.sec}."
            f"{best_depth_msg.header.stamp.nanosec:09d}"
        )
        self._synced_cb(json_msg, best_depth_msg)
    ...
    def _synced_cb(self, json_msg: String, depth_msg: Image) -> None:
        ...
        if not detections:
            self.get_logger().debug("No detections in JSON payload")
            return
    ...
    def _transform_to_map(
        ...
        point = PointStamped()
        if self._use_image_stamp_for_tf:
            point.header.stamp = depth_msg_header_stamp
        else:
        ...
        try:
            ...
            self.get_logger().debug(
                f"TF {source_frame}->{self._target_frame} failed, "
                f"use_image_stamp_for_tf={self._use_image_stamp_for_tf}: {exc}"
            )
            return None
