...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Display semantic_map_offline objects/*.npz in one 3D scene."
    )
...
def write_navigation_json(args: argparse.Namespace, tracks: list[dict]) -> Path:
    ...
    write_semantic_object_map(
        ...
        records,
        frame_id="map",
        source="semantic_map_offline_viewer",
        metadata={
...
def main() -> None:
    ...
    visualizer.create_window(
        window_name="semantic_map_offline semantic objects", width=1440, height=900
    )
