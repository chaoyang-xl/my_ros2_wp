...
| `lc_resnap_max_attempts` | 4 | 失败对象跨新地图帧最大重试次数 |
| `lc_resnap_radius_step_m` | 0.15 | 每轮失败后的搜索半径增量 |
| `lc_resnap_max_radius_m` | 0.6 | 搜索半径上限，避免吸到远处墙体 |
...
仓库脚本：`my_work_pkg/my_work_pkg/monitor.sh`。它按采样间隔计算实时 CPU
占用，并动态读取 RK3588 NPU、GPU、温度、内存、磁盘和高负载进程。

部署到板子后运行（读取 NPU debugfs 需要 sudo）：

```bash
sudo bash /path/to/my_work_pkg/my_work_pkg/monitor.sh

# 每 1 秒刷新
sudo bash /path/to/my_work_pkg/my_work_pkg/monitor.sh --interval 1

# 单次采样，适合保存日志或远程检查
sudo bash /path/to/my_work_pkg/my_work_pkg/monitor.sh --once

# 需要持续追加到日志时才关闭刷屏模式
sudo bash /path/to/my_work_pkg/my_work_pkg/monitor.sh --no-clear
```
...
# ---- CPU 调频策略（RK3588 的 policy 通常按集群共享，不是每核一个）----
watch -n1 'for p in /sys/devices/system/cpu/cpufreq/policy*; do \
  echo "$(basename $p) cpus=$(cat $p/related_cpus) freq=$(cat $p/scaling_cur_freq)kHz"; \
done'
