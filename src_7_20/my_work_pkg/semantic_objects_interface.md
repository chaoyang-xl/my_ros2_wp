...
- `source=snapped`：先做几何修正并进入 `pending_resnap`，等待新 `/map` 和 TF 稳定后再小半径 re-snap。
...
新地图和 TF 稳定条件可以按任意顺序到达。re-snap 失败时保留几何修正位置，
不会修改 `times_seen`、`confidence`、`last_seen`、`state` 或姿态信息。
