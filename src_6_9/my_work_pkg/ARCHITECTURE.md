...
Cartographer 回环时更新 `map → odom` 的 TF 变换。本节点用这个变换的前后差异来反推坐标修正：

```
回环前: T_old: map_old → odom    (记录的值)
回环后: T_new: map_new → odom    (当前值)

对于世界中的一个固定点 P：
  P_odom = T_old(P_old_map)      点在世界中的真实位置不变
  P_odom = T_new(P_new_map)      但 map 坐标系改变了
...
| `lc_translation_threshold` | 0.05 | 位移跳变阈值 (米) — 超过即判定回环 |
| `lc_rotation_threshold` | 0.02 | 旋转跳变阈值 (弧度) — ≈1.1° |
| `lc_source_frame` | `odom` | 监控 `map → odom` 的变化 |
...
```
[WARN] [LoopClosure] TF jump detected: dtrans=0.123m, drot=0.034rad.
...
| `lc_translation_threshold` | 0.05 | 位移跳变阈值 (米) | 太小→正常漂移也被误判；太大→小范围回环漏检 |
| `lc_rotation_threshold` | 0.02 | 旋转跳变阈值 (弧度) | ≈1.1°；同理，根据传感器噪声调节 |
| `lc_source_frame` | `odom` | 监控参考帧 | 默认监控 `map→odom`；如果 TF 树不同可改为 `base_footprint` |
...
2. **检查 TF 监控是否正常工作**：观察节点日志，是否有 `Initialised map->odom reference` 这条消息
3. **检查阈值设置**：如果回环修正没有触发，尝试降低阈值 `-p lc_translation_threshold:=0.02`
4. **确认 TF 树中有 `map → odom`**：Cartographer 必须发布此变换。`ros2 run tf2_tools view_frames` 确认
